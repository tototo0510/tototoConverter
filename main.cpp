...
// 関数名抽出アルゴリズム(C++)
// " "でsplitして 最後の要素に) が含まれていなければ、それが関数名(C++)
// 含まれていた時は、(を含むやつを探す。(を含むやつを(でsplitする。

// 関数名抽出アルゴリズム(C#, understand)
// .でsplitして、最後の要素に)が含まれていなければ、それが関数名(C#, understand)
// 含まれていた時は、(を含むやつを探す。(を含むやつを(でsplitする。
...
// TEC側を読み込む
// [1] ファイル名とIDの辞書
// [2] 関数名とLOCの構造体を持つファイルごとのリスト
// [3] [1]のIDをキーとして[2]を持つリスト
